...
static void __setup()
{
        ...
        if(__class == NULL) {
                ...
                __class = (*__jni_env)->NewGlobalRef(__jni_env, (*__jni_env)->FindClass(__jni_env, "com/manhtran/nativeui/CustomView"));


                __static_method_create  = (*__jni_env)->GetStaticMethodID(__jni_env,
                        __class,
                        "create",
                        "(JLandroid/content/Context;)Lcom/manhtran/nativeui/CustomView;"
                );
        }
}

jobject custom_view_alloc(void *np)
{
        JNIEnv *__jni_env = __jni_env_current_thread();
        __setup();
